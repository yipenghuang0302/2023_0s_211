...
def test_cacheOblivious ( n, test_name, path="./", verbose=False ):
    ...
    try:
        ...
        assert result_tallies[1] < answer_tallies[1]/2, "Cache misses need to be less than half of baseline."
        assert result_tallies[2] < answer_tallies[2]/2, "Cache evictions need to be less than half of baseline."
        return True
    except subprocess.CalledProcessError as e:
        print (e.output)
        print ("Calling ./cacheOblivious returned non-zero exit status.")
